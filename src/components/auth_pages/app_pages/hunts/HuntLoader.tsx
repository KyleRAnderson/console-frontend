...
export default function HuntLoader({
  hunt,
  ...routeProps
}: Props): JSX.Element {
  ...
  const { [AppPaths.HUNT_ID_PARAM]: huntId } = useParams<{
    [AppPaths.HUNT_ID_PARAM]: string;
  }>();

  function onError(): void {
    createNotification({
      message: "Failed to load hunt data.",
      type: "danger",
    });
    history.goBack();
  }
  ...
  useEffect(() => {
    ...
    if (
      newHuntId !== previousHuntId.current &&
      (previousHuntId.current !== undefined || !hunt)
    ) {
      loadHunt();
    } else {
      setLoadedHunt(hunt);
